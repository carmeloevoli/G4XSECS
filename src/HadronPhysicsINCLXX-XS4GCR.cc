...
// ********************************************************************
//
// $Id: MyHadronPhysicsINCLXX.cc 66892 2013-01-17 10:57:59Z gunter $
//
//---------------------------------------------------------------------------
//
...
// 27.11.2011 P.Kaitaniemi: Created physics list for INCL++ using QGSP_INCL_ABLA
// as a template
//
//----------------------------------------------------------------------------
//
#include "HadronPhysicsINCLXX-XS4GCR.hh"
...
//
G4_DECLARE_PHYSCONSTR_FACTORY(MyHadronPhysicsINCLXX);

G4ThreadLocal MyHadronPhysicsINCLXX::ThreadPrivate*
    MyHadronPhysicsINCLXX::tpdata = 0;

MyHadronPhysicsINCLXX::MyHadronPhysicsINCLXX(G4int)
    : G4VPhysicsConstructor("hInelastic INCLXX"),
      QuasiElastic(true),
      withNeutronHP(false),
      withFTFP(false) {}

MyHadronPhysicsINCLXX::MyHadronPhysicsINCLXX(const G4String& name,
                                             const G4bool quasiElastic,
                                             const G4bool neutronHP,
                                             const G4bool ftfp)
...
void MyHadronPhysicsINCLXX::CreateModels() {
    ...
    if (withFTFP) {
        ...
        tpdata->theFTFPNeutron->SetMinEnergy(EMax - 5. * GeV);
    } else {
        tpdata->theNeutrons->RegisterMe(
            tpdata->theQGSPNeutron = new G4QGSPNeutronBuilder(quasiElasticQGS));
        tpdata->theQGSPNeutron->SetMinEnergy(EMax - 5. * GeV);
    }
    tpdata->theNeutrons->RegisterMe(tpdata->theINCLXXNeutron =
                                        new G4INCLXXNeutronBuilder);
    tpdata->theINCLXXNeutron->SetMaxEnergy(EMax);
    if (withNeutronHP) {
        tpdata->theINCLXXNeutron->UsePreCompound(false);
        tpdata->theINCLXXNeutron->SetMinEnergy(19.9 * MeV);
        tpdata->theNeutrons->RegisterMe(tpdata->theNeutronHP =
                                            new G4NeutronPHPBuilder);
    } else {
    ...
    if (withFTFP) {
        ...
        tpdata->theFTFPPro->SetMinEnergy(EMax - 5. * GeV);
    } else {
        tpdata->thePro->RegisterMe(
            tpdata->theQGSPPro = new G4QGSPProtonBuilder(quasiElasticQGS));
        tpdata->theQGSPPro->SetMinEnergy(EMax - 5. * GeV);
    }
    tpdata->thePro->RegisterMe(tpdata->theINCLXXPro =
                                   new G4INCLXXProtonBuilder);
    tpdata->theINCLXXPro->SetMinEnergy(1.0 * MeV);
    tpdata->theINCLXXPro->SetMaxEnergy(EMax);

    tpdata->thePion = new G4PionBuilder;
    if (withFTFP) {
        tpdata->thePion->RegisterMe(tpdata->theFTFPPion =
                                        new G4FTFPPionBuilder(quasiElasticFTF));
        tpdata->theFTFPPion->SetMinEnergy(EMax - 5. * GeV);
    } else {
        tpdata->thePion->RegisterMe(tpdata->theQGSPPion =
                                        new G4QGSPPionBuilder(quasiElasticQGS));
        tpdata->theQGSPPion->SetMinEnergy(EMax - 5. * GeV);
    }
    tpdata->thePion->RegisterMe(tpdata->theINCLXXPion =
                                    new G4INCLXXPionBuilder);
    tpdata->theINCLXXPion->SetMinEnergy(0.0 * GeV);
    tpdata->theINCLXXPion->SetMaxEnergy(EMax);
...
void MyHadronPhysicsINCLXX::ConstructParticle() {
...
void MyHadronPhysicsINCLXX::ConstructProcess() {
    ...
    G4VCrossSectionDataSet* kaonxs =
        new G4CrossSectionInelastic(tpdata->xsKaon);
