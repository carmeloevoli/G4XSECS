...
// ********************************************************************
//
// $Id: MyIonQMDPhysics.cc 80671 2014-05-06 13:59:16Z gcosmo $
//
//---------------------------------------------------------------------------
//
// ClassName:   MyIonQMDPhysics
...
#include "IonQMDPhysics-XS4GCR.hh"
...
//
G4_DECLARE_PHYSCONSTR_FACTORY(MyIonQMDPhysics);

G4ThreadLocal std::vector<G4HadronInelasticProcess*>* MyIonQMDPhysics::p_list =
    0;
G4ThreadLocal std::vector<G4HadronicInteraction*>* MyIonQMDPhysics::model_list =
    0;

G4ThreadLocal G4VCrossSectionDataSet* MyIonQMDPhysics::theNuclNuclData = 0;
G4ThreadLocal G4VComponentCrossSection* MyIonQMDPhysics::theGGNuclNuclXS = 0;

G4ThreadLocal G4BinaryLightIonReaction* MyIonQMDPhysics::theIonBC = 0;
G4ThreadLocal G4HadronicInteraction* MyIonQMDPhysics::theFTFP = 0;
G4ThreadLocal G4FTFBuilder* MyIonQMDPhysics::theBuilder = 0;
G4ThreadLocal G4QMDReaction* MyIonQMDPhysics::theQMD = 0;
G4ThreadLocal G4bool MyIonQMDPhysics::wasActivated = false;

MyIonQMDPhysics::MyIonQMDPhysics(G4int ver)
    ...
    if (verbose > 1) G4cout << "### MyIonQMDPhysics" << G4endl;
}

MyIonQMDPhysics::MyIonQMDPhysics(const G4String& name, G4int ver)
...
MyIonQMDPhysics::~MyIonQMDPhysics() {
    if (wasActivated) {
        delete theBuilder;
        theBuilder = 0;
...
void MyIonQMDPhysics::ConstructProcess() {
    ...
    theNuclNuclData = new G4CrossSectionInelastic(
    ...
    AddProcess("ionInelastic", G4GenericIon::GenericIon(), theIonBC, theQMD,
               theFTFP);
}

void MyIonQMDPhysics::AddProcess(const G4String& name, G4ParticleDefinition* p,
    ...
    if (verbose > 1) {
        G4cout << "Register " << hadi->GetProcessName() << " for "
               << p->GetParticleName() << G4endl
               << "       Binary Cascade for E(MeV)= " << eminBIC << " - "
               << (QMD == 0 ? emaxQMD : (eminQMD - overlap));
        if (QMD) {
            G4cout << G4endl << "       QMD for E(MeV)= " << eminQMD << " - "
                   << emaxQMD;
        }
        if (FTFP) {
            G4cout << G4endl << "       FTFP for E(MeV)= " << emaxQMD - overlap
                   << " - " << emaxFTFP;
        }
        G4cout << G4endl;
    }
...
void MyIonQMDPhysics::ConstructParticle() {
