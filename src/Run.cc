...
void Run::SetTargetXXX(G4bool flag) { fTargetXXX = flag; }
...
void Run::EndOfRun(G4bool print) {
    ...
    if (material->GetNumberOfElements() == 1) {
        ...
        for (it = fProcCounter.begin(); it != fProcCounter.end(); it++) {
            G4String procName = it->first;
            G4VProcess* process =
                processTable->FindProcess(procName, fParticle);
            G4double xs1 = store->GetCrossSectionPerVolume(fParticle, fEkin,
                                                           process, material);
            G4double massSigma = xs1 / density;
            sumc1 += massSigma;
            G4double xs2 = store->GetCrossSectionPerAtom(
                fParticle, fEkin, process, element, material);
            sumc2 += xs2;
            G4cout << "\n"
        ...
        for (it = fProcCounter.begin(); it != fProcCounter.end(); it++) {
            ...
            G4VProcess* process =
                processTable->FindProcess(procName, fParticle);
            G4double xs = store->GetCrossSectionPerVolume(fParticle, fEkin,
                                                          process, material);
    ...
    for (ic = fNuclChannelMap.begin(); ic != fNuclChannelMap.end(); ic++) {
        ...
        if (print)
            G4cout << "  " << std::setw(60) << name << ": " << std::setw(7)
                   << count << "   Q = " << std::setw(wid)
                   << G4BestUnit(Q, "Energy") << G4endl;
    }
    ...
    if (print && (fGammaCount > 0)) {
        G4cout << "\n"
               << std::setw(58)
               << "number of gamma or e- (ic): N = " << fNbGamma[1] << " --> "
               << fNbGamma[2] << G4endl;
    }

    if (print && fTargetXXX) {
        G4cout << "\n   --> NOTE: XXXX because neutronHP is unable to return "
                  "target nucleus"
               << G4endl;
    }
    ...
    for (itn = fParticleDataMap.begin(); itn != fParticleDataMap.end(); itn++) {
        ...
        G4double eMax = data.fEmax;
        if (print)
            G4cout << "  " << std::setw(13) << name << ": " << std::setw(7)
                   << count << "  Emean = " << std::setw(wid)
    ...
    if (fTotalCount > 1) {
        ...
        G4cout << "\n   Momentum balance: Pmean = " << std::setw(wid)
               << G4BestUnit(Pbmean, "Energy") << "\t( "
