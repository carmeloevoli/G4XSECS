...
HISTO::HISTO(TTree *tree) : fChain(0) {
    ...
    if (tree == 0) {
        TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject("xsec.root");
        if (!f || !f->IsOpen()) {
...
void HISTO::Init(TTree *tree) {
    ...
    fChain->SetBranchAddress("NDecayChannels", &NDecayChannels,
                             &b_NDecayChannels);
...
void HISTO::Loop(const Bool_t drawExp) {
    ...
    cout << nentries << " " << nPrim << " " << fChain->GetMinimum("EventNumber")
         << " " << EMinPrimary << " " << EMaxPrimary << "\n";
