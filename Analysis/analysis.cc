...
void analysis(char* filename) {
    ...
    TH1D* hBRnorm = new TH1D("hBRnorm", "total number of events vs energy",
                             Nbins, lgEmin, lgEmax);
    hBRnorm->GetXaxis()->SetTitle("log_{10}(E/MeV)");
    hBRnorm->Sumw2();

    TH1D* hXsecNominal =
        new TH1D("hXsecNominal", "inelastic X-section vs energy ", Nbins,
                 lgEmin, lgEmax);
    ...
    if (DEBUG)
        cout << " Nbins -> " << Nbins << " " << lgEmin << " " << lgEmax << endl;
    ...
    for (int i = 0; i < np; i++) {
        ...
        if (DEBUG)
            for (int k = 1; k <= Nbins; k++)
                cout << k << " " << hBR[i]->GetBinContent(k) << " "
                     << hBR[i]->GetBinCenter(k) << endl;
    }

    if (DEBUG)
        for (int k = 1; k <= Nbins; k++)
            cout << k << " " << hBRnorm->GetBinContent(k) << " "
                 << hBRnorm->GetBinCenter(k) << endl;
    ...
    for (int i = 1; i <= Nbins; i++) {
        myOut << std::fixed << std::setw(8) << std::setprecision(5)
              << hXsec[0]->GetBinCenter(i) << " ";
        for (int j = 0; j < np; j++) {
            myOut << std::fixed << std::setw(8) << std::setprecision(5)
                  << hXsec[j]->GetBinContent(i) << " " << std::fixed
                  << std::setw(8) << std::setprecision(3)
                  << hXsec[j]->GetBinError(i) << " ";
        }
        myOut << endl;
    }
